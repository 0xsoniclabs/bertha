...
func dumpTransactions(data []*types.Transaction) string {
	...
	rustCode.WriteString(fmt.Sprintf("pub const TRANSACTION_ROOT : &str = \"%s\";\n", transactionRoot))
	...
	`)
	rustCode.WriteString("pub fn generate_transactions_with_data() -> impl IntoIterator<Item = TransactionWithTestData> {\n")
	rustCode.WriteString("[\n")
	for _, tx := range data {
		...
		rustCode.WriteString("TransactionWithTestData {\n")
		rustCode.WriteString(fmt.Sprintf("\t\ttransaction: %s,\n", rustTx))
		rustCode.WriteString(fmt.Sprintf("\t\trlp_encoding: const_hex::decode(\"%s\").unwrap(),\n", common.Bytes2Hex(encoding)))
		rustCode.WriteString(fmt.Sprintf("\t\tjson_representation: r#\"%s\"#.to_string(),\n", string(jsonRepresentation)))
		rustCode.WriteString("},\n")
	}
	rustCode.WriteString("]\n}")
	return rustCode.String()
}
...
func dumpReceipts(data []*types.Receipt) string {
    ...
    use crate::{Address, Hash, Bloom, HexConvert, Log, TransactionReceipt, TransactionType, receipts::PostStateOrStatus};
	...
	rustCode.WriteString(fmt.Sprintf("pub const RECEIPTS_ROOT : &str = \"%s\";\n\n", receiptsRoot))
	...
	`)
	rustCode.WriteString("pub fn generate_receipts_with_data() -> impl IntoIterator<Item = TransactionReceiptWithTestData> {\n")
	rustCode.WriteString("[\n")
	for _, receipt := range data {
		...
		rustCode.WriteString("TransactionReceiptWithTestData {\n")
		rustCode.WriteString(fmt.Sprintf("\t\treceipt: %s,\n", rustReceipt))
		rustCode.WriteString(fmt.Sprintf("\t\tbloom: %s,\n", toRustBloom(receipt)))
		rustCode.WriteString(fmt.Sprintf("\t\trlp_encoding: const_hex::decode(\"%s\").unwrap(),\n", common.Bytes2Hex(encoding)))
		rustCode.WriteString(fmt.Sprintf("\t\t\t\tjson_representation: r#\"%s\"#.to_string(),\n", string(jsonRepresentation)))
		rustCode.WriteString("},\n")
	}
...
// dumpWithdrawals generates a Rust iterator over withdrawals with test data.
// It includes:
// 1. The withdrawal
// 2. The withdrawal RLP encoding
// 3. The withdrawal JSON representation
func dumpWithdrawals(data []*types.Withdrawal) string {
	var rustCode strings.Builder
	rustCode.WriteString(`
	// This file is generated by go/test_data_generator/main.go
	#![allow(dead_code)]
	...
	`)
	// Generate the Withdrawal struct
	rustCode.WriteString(`
	#[derive(Debug, Clone)]
	...
	`)
	rustCode.WriteString("pub fn generate_withdrawals_with_data() -> impl IntoIterator<Item = WithdrawalWithTestData> {\n")
	rustCode.WriteString("[\n")
	for _, withdrawal := range data {
		rustW := toRustWithdrawal(withdrawal)
		encoding, _ := rlp.EncodeToBytes(withdrawal)
		jsonRepresentation, _ := json.Marshal(withdrawal)
		rustCode.WriteString("WithdrawalWithTestData {\n")
		rustCode.WriteString(fmt.Sprintf("\t\twithdrawal: %s,\n", rustW))
		rustCode.WriteString(fmt.Sprintf("\t\trlp_encoding: const_hex::decode(\"%s\").unwrap(),\n", common.Bytes2Hex(encoding)))
		rustCode.WriteString(fmt.Sprintf("\t\tjson_representation: r#\"%s\"#.to_string(),\n", string(jsonRepresentation)))
		rustCode.WriteString("},\n")
	}
...
func dumpBlocks(data []BlockWithReceipts) string {
	...
	// This file is generated by go/test_data_generator/main.go
	 #![allow(dead_code)]
	use crate::{Address, Hash, HexConvert, Block, Transaction, TransactionReceipt, TransactionType, U256, AccessListEntry, SetCodeAuthorization, Log, Withdrawal, receipts::PostStateOrStatus};
	`)
	...
	`)
	rustCode.WriteString("pub fn generate_blocks_with_data() -> impl IntoIterator<Item = BlockWithTestData> {\n")
	rustCode.WriteString("[\n")
	for _, block := range data {
		...
		rustCode.WriteString("BlockWithTestData {\n")
		rustCode.WriteString(fmt.Sprintf("\t\tblock: %s,\n", rustBlock))
		rustCode.WriteString(fmt.Sprintf("\t\tblock_hash: Hash::try_from_hex(\"%s\").unwrap(),\n", block.Block.Hash().Hex()))
		rustCode.WriteString(fmt.Sprintf("\t\ttransaction_root: Hash::try_from_hex(\"%s\").unwrap(),\n", transactionRoot))
		rustCode.WriteString(fmt.Sprintf("\t\treceipts_root: Hash::try_from_hex(\"%s\").unwrap(),\n", receiptsRoot))
...
func main() {
	...
	case "withdrawals":
		...
		formatAndPrintRustCode(dumpWithdrawals(withdrawals))
	default:
