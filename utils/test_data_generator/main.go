...
func dumpWithdrawals(data []*types.Withdrawal) string {
	...
	`
	rustCode += "pub fn generate_withdrawals_with_data() -> impl IntoIterator<Item = WithdrawalWithTestData> {\n"
	rustCode += "[\n"
	for _, withdrawal := range data {
		...
		rustCode += "WithdrawalWithTestData {\n"
		rustCode += fmt.Sprintf("\t\twithdrawal: %s,\n", rustW)
		rustCode += fmt.Sprintf("\t\trlp_encoding: const_hex::decode(\"%s\").unwrap(),\n", common.Bytes2Hex(encoding))
		rustCode += fmt.Sprintf("\t\tjson_representation: r#\"%s\"#.to_string(),\n", string(jsonRepresentation))
		rustCode += "},\n"
	}
