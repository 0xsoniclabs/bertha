...
// transaction hashes.
func RPCMarshalBlock(block *types.Block, inclTx bool, fullTx bool, receipts []*types.Receipt, config *params.ChainConfig) map[string]any {
	...
	formattedReceipts := make([]any, len(receipts))
...
func NewRPCTransactionWithoutBlock(tx *types.Transaction) *RPCTransaction {
	// Create a transaction without block metadata
	return newRPCTransaction(tx, common.Hash{}, 0, 0, 0, nil, params.MainnetChainConfig)
}
