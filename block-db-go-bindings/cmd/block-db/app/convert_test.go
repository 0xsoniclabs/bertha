...
func TestConvertToGethBlock_ConvertsBlockToGethBlock(t *testing.T) {
	...
	want := (&types.Block{}).
		WithSeal(&types.Header{
			ParentHash:      common.BytesToHash(input.ParentHash),
			UncleHash:       common.BytesToHash(input.OmmersHash),
			Coinbase:        common.BytesToAddress(input.Beneficiary),
			Root:            common.BytesToHash(input.StateRoot),
			Difficulty:      new(big.Int).SetUint64(input.Difficulty),
			Number:          new(big.Int).SetUint64(input.Number),
			GasLimit:        input.GasLimit,
			GasUsed:         receipts[len(receipts)-1].CumulativeGasUsed,
			Time:            input.Timestamp,
			Extra:           input.ExtraData,
			MixDigest:       common.BytesToHash(input.PrevRandao),
			Nonce:           types.BlockNonce(input.Nonce),
			BaseFee:         new(big.Int).SetBytes(input.BaseFeePerGas),
			WithdrawalsHash: toOptionalHash(input.WithdrawalsRoot),
			BlobGasUsed:     input.BlobGasUsed,
			ExcessBlobGas:   input.ExcessBlobGas,
